...
let messageIndex = 0; // 0=no message yet, 1=first message, 2=second, 3=third
...
// ==================== GIFT BOXES WITH RANDOM EMPTY BOXES ====================
let giftBoxMessages = [];
let giftBoxOrder = [];
...
function initBalloons() {
    ...
    let gifts = [];
    for (let i = 0; i < 7; i++) {
        gifts.push({ index: i, type: 'empty', message: '' });
    }
    
    // Randomly select 3 positions for message boxes
    let msgPositions = [];
    while (msgPositions.length < 3) {
        let pos = Math.floor(Math.random() * 7);
        if (!msgPositions.includes(pos)) {
            msgPositions.push(pos);
        }
    }
    msgPositions.sort((a, b) => a - b);
    
    // Assign messages to the 3 positions in order
    gifts[msgPositions[0]].type = 'msg1';
    gifts[msgPositions[0]].message = `You have answered ${riddleCorrectCount} out of 3 riddles correctly!`;
    
    gifts[msgPositions[1]].type = 'msg2';
    gifts[msgPositions[1]].message = `I will send you 5 ${foodChoice} 🍫 for each correct question!`;
    
    gifts[msgPositions[2]].type = 'msg3';
    gifts[msgPositions[2]].message = riddlesCorrectCount >= 2 ? "You are damn smart 🤓" : "But still, you are too slow 🐢😂";
    
    // Store the order of messages
    giftBoxOrder = [...msgPositions];
    
    // Create floating balloons with gift boxes
    for (let i = 0; i < 7; i++) {
        const giftDiv = document.createElement('div');
        giftDiv.className = 'balloon-gift-item';
        giftDiv.setAttribute('data-index', i);
        giftDiv.setAttribute('data-type', gifts[i].type);
        giftDiv.setAttribute('data-message', gifts[i].message);
        giftDiv.innerHTML = `<div class="balloon-icon">🎈</div><div class="gift-icon">🎁</div>`;
        giftDiv.onclick = () => openGiftBox(i, gifts[i].type, gifts[i].message);
...
function openGiftBox(index, type, message) {
    ...
    if (type === 'empty') {
        ...
        // Show the message in sequence
        if (type === 'msg1' && currentMsgIndex === 0) {
            msgDiv.innerHTML = `<p>🎁 ${message}</p>`;
            currentMsgIndex++;
        } else if (type === 'msg2' && currentMsgIndex === 1) {
            msgDiv.innerHTML = `<p>🎁 ${message}</p>`;
            currentMsgIndex++;
        } else if (type === 'msg3' && currentMsgIndex === 2) {
            msgDiv.innerHTML = `<p>🎁 ${message}</p>`;
            currentMsgIndex++;
        } else {
            // If not the expected next message, show try again
            msgDiv.innerHTML = `<p>😜 Try again in order! 😜</p>`;
            giftOpened.pop(); // Remove from opened since it wasn't the right order
            document.getElementById('popped-count').innerText = giftOpened.length;
            return;
        }
    }
    
    // Add visual effect to clicked gift
